...
func (agc AsyncGithubClient) GetPullRequestInfoFull(info PRInfo) (PRResponseInfo, error) {
	prInfoList := map[int]PRInfo{}
	prInfoList[info.Id] = info
	prs, err := agc.GetPullRequestInfoFullMultiple(prInfoList)
	return *prs[info.Id], err
}

func (agc AsyncGithubClient) GetPullRequestInfoFullMultiple(infos map[int]PRInfo) (map[int]*PRResponseInfo, error) {
