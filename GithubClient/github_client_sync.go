...
func (sgc SyncGithubClient) GetPullRequestInfoFull(info PRInfo) (PRResponseInfo, error) {
	prInfoList := map[int]PRInfo{}
	prInfoList[info.Id] = info
	prs, err := sgc.GetPullRequestInfoFullMultiple(prInfoList)
	return *prs[info.Id], err
}

func (sgc SyncGithubClient) GetPullRequestInfoFullMultiple(infos map[int]PRInfo) (map[int]*PRResponseInfo, error) {
